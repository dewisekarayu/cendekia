    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-400 mb-3">
                <a href="<?php echo e(route('mahasiswa.dashboard')); ?>" class="hover:text-blue-600">Dashboard</a>
            ...
            <div class="flex items-start justify-between gap-4">
                ...
                <a href="<?php echo e(route('mahasiswa.dashboard')); ?>" class="flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-700 dark:text-slate-200 rounded-lg sm:rounded-xl font-medium transition text-sm sm:text-base flex-shrink-0">
                        ...
                        <div class="mt-3 sm:mt-4 overflow-x-auto -mx-5 sm:mx-0 px-5 sm:px-0">
                            <div class="flex gap-1.5 sm:gap-2 pb-2 flex-nowrap">
                                <?php $__currentLoopData = [
                                    ...
                                    <label class="flex items-center gap-1 cursor-pointer px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg transition text-xs sm:text-sm whitespace-nowrap flex-shrink-0" :class="visibleCategories.has('<?php echo e($cat); ?>') ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-gray-50 dark:bg-slate-700/50 hover:bg-gray-100 dark:hover:bg-slate-700'">
                                        <input type="checkbox" 
                                            @click="toggleCategory('<?php echo e($cat); ?>')"
                                            :checked="visibleCategories.has('<?php echo e($cat); ?>')"
                                            class="w-3 h-3 sm:w-4 sm:h-4 rounded accent-blue-600 cursor-pointer">
                                        <span class="font-medium" :class="visibleCategories.has('<?php echo e($cat); ?>') ? 'text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300'"><?php echo e($label); ?></span>
                                    </label>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        </div>
                    ...
                    <div class="p-3 sm:p-5 overflow-x-auto">
                        <div class="grid grid-cols-7 gap-0.5 sm:gap-1 mb-2 sm:mb-3 min-w-max">
                            <?php $__currentLoopData = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="text-center text-xs sm:text-sm font-bold text-gray-600 dark:text-gray-400 py-1.5 sm:py-2"><?php echo e($day); ?></div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
...
<?php $__env->startPush('scripts'); ?>
    ...
    console.log('Total Events:', <?php echo json_encode($events, 15, 512) ?>->length || 0);
    console.log('Events by Date:', Object.keys(<?php echo json_encode($eventsByDate, 15, 512) ?> || {}).length || 0);
    console.log('Selected Semester:', "<?php echo e($selectedSemesterId ?? ''); ?>");
    console.log('Current Month/Year:', "<?php echo e($selectedMonth); ?>/<?php echo e($selectedYear); ?>");
    console.log('Event Categories:', <?php echo json_encode(array_unique(array_column($events->toArray(), 'jenis_kegiatan')), 512) ?> || []);
