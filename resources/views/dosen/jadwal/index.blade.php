...
@section('content')
    ...
    @if($kelasPerkuliahan->isEmpty())
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-8 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-700 mb-2">Belum ada jadwal mengajar</h3>
            <p class="text-gray-500 mb-6">Anda belum ditugaskan sebagai dosen pengampu pada kelas perkuliahan aktif.</p>
        </div>
    @else
        <!-- Statistik -->
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-1">
                                                ...
                                                @endif
                                                <span class="bg-green-100 text-green-800 text-xs font-medium px-2 py-0.5 rounded">{{ $kelas->jumlah_mahasiswa }}/{{ $kelas->kuota_mahasiswa }} Mahasiswa</span>
                                            </div>
                                            ...
                                            <div class="flex flex-wrap gap-4 text-sm text-gray-600 mb-2">
                                                ...
                                                @if($kelas->programStudi)
                                                <div class="flex items-center">
                                                    <svg class="w-4 h-4 mr-1 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                                                    </svg>
                                                    <span>{{ $kelas->programStudi->nama_prodi }}</span>
                                                </div>
                                                @endif
                                            </div>
                                            ...
                                            <div class="text-xs text-gray-500">
                                                <span class="inline-flex items-center">
                                                    ...
                                                    • Status: {{ ucfirst($kelas->status_kelas) }}
                                                </span>
                                            </div>
                                            
                                            <!-- Team Teaching Info -->
                                            @if($kelas->dosen_pengampu && count($kelas->dosen_pengampu) > 0)
                                                <div class="mt-2">
                                                    <p class="text-xs font-medium text-gray-700 mb-1">Team Teaching:</p>
                                                    <div class="flex flex-wrap gap-1">
                                                        @foreach($kelas->semuaDosenPengampu() as $dosen)
        ...
        <div class="mt-8 flex flex-wrap gap-3 justify-between items-center">
            <div class="flex gap-2">
                <a href="{{ route('dosen.jadwal.calendar') }}" class="inline-flex items-center bg-blue-600 text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-blue-700 transition">
                ...
                <button onclick="window.print()" class="inline-flex items-center bg-gray-100 text-gray-700 text-sm font-medium px-4 py-2 rounded-lg hover:bg-gray-200 transition">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('dosen.jadwal.export') }}" class="text-blue-600 hover:text-blue-800 font-medium text-sm">
                Export ke PDF
            </a>
        </div>
    @endif
