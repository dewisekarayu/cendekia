    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#321270] via-[#461a9c] to-[#5a2cc9] px-6 py-8 sm:px-8 shadow-md">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div class="flex-1">
                ...
                <div id="ai_insight_container" class="mt-4 mb-2 relative overflow-hidden rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 p-5 flex items-start gap-4 transition-opacity duration-300 shadow-[0_8px_30px_rgb(0,0,0,0.12)] before:absolute before:inset-0 before:bg-gradient-to-r before:from-transparent before:via-white/5 before:to-transparent">
                    <div class="relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-400 to-purple-500 shadow-lg shadow-purple-500/30 ring-2 ring-white/20">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-white animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                    </div>
                    <div class="relative z-10 flex-1">
                        <div class="flex items-center gap-2 mb-2">
                            <span class="text-[10px] font-black text-fuchsia-200 uppercase tracking-widest bg-fuchsia-500/20 px-2 py-0.5 rounded-full border border-fuchsia-400/30 shadow-sm">CENDEKIA AI INSIGHT</span>
                        ...
                        <p id="ai_insight_text" class="text-[13px] font-medium leading-relaxed text-purple-50 mt-1">
                            <span class="animate-pulse inline-flex items-center gap-2 text-white/70">
                                Menganalisis aktivitas Anda hari ini...
                            </span>
                        </p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-3 gap-3 w-full sm:w-72 shrink-0">
        ...
        @foreach ([
            ...
            <div class="flex items-center gap-4 rounded-2xl border border-slate-200/60 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm transition-colors duration-200">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800 dark:text-white mt-0.5">{{ number_format($stat['value'], 0, ',', '.') }}</p>
                </div>
            </div>
        @endforeach
    </div>
                        ...
                        <div class="flex flex-1 flex-col p-5">
                            ...
                            <div class="mt-4 space-y-2 border-t border-slate-50 dark:border-slate-700 pt-3 text-xs text-slate-500 dark:text-slate-400">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="font-medium text-slate-600 dark:text-slate-300">{{ $kelas->hari }}, {{ substr($kelas->jam_mulai, 0, 5) }} WIB</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    ...
                                    <span class="font-medium text-slate-600 dark:text-slate-300">{{ $kelas->mahasiswa->count() }} Mahasiswa Terdaftar</span>
                                </div>
                            </div>
                            ...
                            <tr class="hover:bg-purple-50/10 dark:hover:bg-purple-900/10 transition duration-150">
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-semibold text-slate-700 dark:text-slate-200 max-w-[140px] truncate block">{{ $item->mahasiswa?->name ?? '-' }}</span>
                                    </div>
                                </td>
                                <td class="px-5 py-3.5">
                                    <p class="font-bold text-[#321270] dark:text-purple-300 max-w-[200px] truncate">{{ $item->tugas?->judul ?? '-' }}</p>
                                    <p class="text-[11px] font-medium text-slate-400 dark:text-slate-500 max-w-[200px] truncate mt-0.5">{{ $item->tugas?->kelasPerkuliahan?->mataKuliah?->nama_mk ?? '-' }}</p>
                                </td>
