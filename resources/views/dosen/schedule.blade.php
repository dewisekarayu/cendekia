                    ...
                    <div class="space-y-3">
                        ...
                        @foreach ($schedules as $item)
                            <div class="p-4 rounded-lg {{ $item['color'] }} flex items-start justify-between hover:shadow-md transition cursor-pointer">
                                <div class="flex-1">
                                    <p class="text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1">{{ $item['time'] }}</p>
                                    <h4 class="text-sm font-bold text-slate-800 dark:text-white">{{ $item['class'] }}</h4>
                ...
                <div class="space-y-3 p-5">
                    ...
                    @foreach ($events as $event)
                        <div class="flex gap-3 pb-3 border-b border-gray-100 dark:border-slate-700 last:border-0">
                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg {{ $event['type'] == 'Ujian' ? 'bg-red-50 dark:bg-red-950/40' : ($event['type'] == 'Rapat' ? 'bg-blue-50 dark:bg-blue-950/40' : 'bg-amber-50 dark:bg-amber-950/40') }}">
                            ...
                            <div class="flex-1">
                                <p class="text-xs text-gray-500 dark:text-slate-400 font-semibold">{{ $event['date'] }}</p>
                                <p class="text-sm font-bold text-gray-800 dark:text-white mt-0.5">{{ $event['title'] }}</p>
                                <span class="inline-block mt-1 text-[10px] font-semibold {{ $event['type'] == 'Ujian' ? 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40' : ($event['type'] == 'Rapat' ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40' : 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40') }} px-2 py-0.5 rounded">
                                    {{ $event['type'] }}
                                </span>
                            </div>
                        </div>
                ...
                <div class="space-y-4 p-5">
                    ...
                    @foreach ($stats as $stat)
                        <div class="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-slate-700 last:border-0">
                            <span class="text-sm text-gray-600 dark:text-slate-300">{{ $stat['label'] }}</span>
                            <div class="text-right">
                                <p class="text-lg font-bold text-slate-800 dark:text-white">{{ $stat['value'] }}</p>
                                <p class="text-xs text-gray-550 dark:text-slate-500">{{ $stat['unit'] }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
