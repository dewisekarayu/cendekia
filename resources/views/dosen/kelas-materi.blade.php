...
<script>
    ...
    async function generateAiDescription(btn) {
        ...
        try {
            const response = await fetch('{{ route("dosen.ai-assistant.generate-desc") }}', {
                method: 'POST',
                headers: {
    ...
    async function generatePdfFromText(btn, type) {
        ...
        if (!judul) {
            alert('Silakan isi Judul terlebih dahulu agar AI mengetahui topik soal/materi yang akan dibuat!');
            return;
        }
