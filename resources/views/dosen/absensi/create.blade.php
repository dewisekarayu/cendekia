    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-200/60">
        <div class="min-w-0">
            <div class="flex items-center gap-2 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
                <a href="{{ route('dosen.kelas-saya') }}" class="hover:text-purple-600 transition duration-200">Kelas Saya</a>
                ...
                <a href="{{ route('dosen.absensi.index', $kelas->id) }}" class="hover:text-purple-600 transition duration-200">Presensi</a>
                <svg class="w-3.5 h-3.5 text-slate-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"/></svg>
            ...
            <h1 class="text-2xl sm:text-3xl font-bold text-slate-800 flex items-center gap-3 mt-2">
                <div class="w-11 h-11 rounded-xl bg-purple-50 text-purple-600 border border-purple-100 shadow-sm flex items-center justify-center flex-shrink-0">
            ...
            <p class="mt-2 text-sm text-slate-500 flex items-center flex-wrap gap-2">
                <span class="font-semibold text-purple-600 bg-purple-50 px-2.5 py-1 rounded-md border border-purple-100/70">{{ $kelas->kode_kelas }}</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200/80 overflow-hidden">
                <div class="bg-gradient-to-r from-slate-50 to-purple-50/30 px-6 py-5 border-b border-slate-100 flex items-center gap-3">
                ...
                <form action="{{ route('dosen.absensi.store', $kelas->id) }}" method="POST" class="p-6 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" id="pertemuan_ke" name="pertemuan_ke"
                                ...
                                class="w-full px-3.5 py-2.5 text-sm text-slate-700 rounded-xl border @error('pertemuan_ke') border-rose-300 bg-rose-50/30 focus:border-rose-500 @else border-slate-200 bg-slate-50 focus:border-purple-500 @enderror focus:bg-white focus:ring-2 focus:ring-purple-500/20 outline-none transition duration-200 shadow-sm">
                            @error('pertemuan_ke')
                        ...
                        <div>
                            ...
                            <input type="date" id="tanggal" name="tanggal"
                                ...
                                required
                                class="w-full px-3.5 py-2.5 text-sm text-slate-700 rounded-xl border @error('tanggal') border-rose-300 bg-rose-50/30 focus:border-rose-500 @else border-slate-200 bg-slate-50 focus:border-purple-500 @enderror focus:bg-white focus:ring-2 focus:ring-purple-500/20 outline-none transition duration-200 shadow-sm">
                            @error('tanggal')
                                <p class="mt-1 text-xs text-rose-600 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="time" id="jam_mulai" name="jam_mulai"
                                ...
                                required
                                class="w-full px-3.5 py-2.5 text-sm text-slate-700 rounded-xl border @error('jam_mulai') border-rose-300 bg-rose-50/30 focus:border-rose-500 @else border-slate-200 bg-slate-50 focus:border-purple-500 @enderror focus:bg-white focus:ring-2 focus:ring-purple-500/20 outline-none transition duration-200 shadow-sm">
                            @error('jam_mulai')
                                <p class="mt-1 text-xs text-rose-600 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="time" id="jam_selesai" name="jam_selesai"
                                ...
                                required
                                class="w-full px-3.5 py-2.5 text-sm text-slate-700 rounded-xl border @error('jam_selesai') border-rose-300 bg-rose-50/30 focus:border-rose-500 @else border-slate-200 bg-slate-50 focus:border-purple-500 @enderror focus:bg-white focus:ring-2 focus:ring-purple-500/20 outline-none transition duration-200 shadow-sm">
                            @error('jam_selesai')
                                <p class="mt-1 text-xs text-rose-600 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <textarea id="rangkuman" name="rangkuman" rows="3"
                            ...
                            class="w-full px-4 py-3 text-sm text-slate-700 bg-slate-50 hover:bg-white focus:bg-white rounded-xl border border-slate-200 focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none transition duration-200 shadow-sm resize-none">{{ old('rangkuman') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="berita_acara" name="berita_acara" rows="3"
                            ...
                            class="w-full px-4 py-3 text-sm text-slate-700 bg-slate-50 hover:bg-white focus:bg-white rounded-xl border border-slate-200 focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none transition duration-200 shadow-sm resize-none">{{ old('berita_acara') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="catatan" name="catatan" rows="2"
                            ...
                            class="w-full px-4 py-3 text-sm text-slate-700 bg-slate-50 hover:bg-white focus:bg-white rounded-xl border border-slate-200 focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none transition duration-200 shadow-sm resize-none">{{ old('catatan') }}</textarea>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-100">
                        ...
                        <button type="submit" 
                                class="w-full sm:w-auto px-8 py-3 bg-gradient-to-r from-purple-600 to-fuchsia-600 hover:from-purple-700 hover:to-fuchsia-700 text-white rounded-xl font-bold text-sm shadow-md hover:shadow-lg transition-all duration-300 hover:scale-[1.01]">
                            ✓ Buat Sesi Baru
                        </button>
                    </div>
                </form>
        ...
        <div class="space-y-4">
            <div class="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm hover:shadow-md transition duration-300">
                <div class="flex items-start gap-3">
                    <div class="w-9 h-9 rounded-lg bg-purple-50 text-purple-600 border border-purple-100 flex items-center justify-center flex-shrink-0">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                ...
                <div class="space-y-3.5">
                    ...
                    <div class="pt-3 border-t border-slate-100">
                        ...
                        <div class="flex items-baseline gap-2">
                            <p class="text-3xl font-extrabold text-purple-600">{{ $kelas->mahasiswa->count() }}</p>
            ...
            <div class="bg-gradient-to-br from-slate-50 to-purple-50/20 rounded-xl border border-slate-200/60 p-5 shadow-sm">
                <div class="flex items-start gap-3">
                    <div class="w-9 h-9 rounded-lg bg-purple-50 text-purple-600 border border-purple-100 flex items-center justify-center flex-shrink-0">
