...
@if (session('success'))
    <div
        ...
        class="mb-5 flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 shadow-sm {{ $class }}"
        role="alert"
    >
...
@if (session('error'))
    <div
        ...
        x-transition:leave-start="opacity-100 translate-y-0"
        x-transition:leave-end="opacity-0 -translate-y-2"
        class="mb-5 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 shadow-sm {{ $class }}"
        role="alert"
    >
...
@if (session('warning'))
    <div
        ...
        x-transition:leave-start="opacity-100 translate-y-0"
        x-transition:leave-end="opacity-0 -translate-y-2"
        class="mb-5 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 shadow-sm {{ $class }}"
        role="alert"
    >
...
@if ($errors->any())
    <div
        ...
        x-transition:leave-start="opacity-100 translate-y-0"
        x-transition:leave-end="opacity-0 -translate-y-2"
        class="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 shadow-sm {{ $class }}"
        role="alert"
    >
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
                <p class="font-semibold mb-1">Terdapat {{ $errors->count() }} kesalahan pada form:</p>
