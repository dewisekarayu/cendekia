...
@section('content')
    ...
    <h1 class="text-xl font-bold text-gray-800 mb-6">Edit Mata Kuliah</h1>

    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 max-w-xl">
        <form action="{{ route('admin.mata-kuliah.update', $mk->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="program_studi_id" class="w-full border-gray-300 rounded-lg text-sm">
                    @foreach ($prodiList as $prodi)
                        <option value="{{ $prodi->id }}" {{ old('program_studi_id', $mk->program_studi_id) == $prodi->id ? 'selected' : '' }}>
                            {{ $prodi->nama_prodi }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="kode_mk" value="{{ old('kode_mk', $mk->kode_mk) }}" class="w-full border-gray-300 rounded-lg text-sm">
            ...
            <div>
                ...
                <input type="text" name="nama_mk" value="{{ old('nama_mk', $mk->nama_mk) }}" class="w-full border-gray-300 rounded-lg text-sm">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="sks" value="{{ old('sks', $mk->sks) }}" min="1" max="6" class="w-full border-gray-300 rounded-lg text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="semester_ke" value="{{ old('semester_ke', $mk->semester_ke) }}" min="1" max="8" class="w-full border-gray-300 rounded-lg text-sm">
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" class="w-full border-gray-300 rounded-lg text-sm">{{ old('deskripsi', $mk->deskripsi) }}</textarea>
