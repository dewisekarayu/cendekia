...
{{--
    PERBAIKAN UTAMA:
    1. Seluruh state kalender (bulan, tahun, filter) dikelola dalam SATU x-data="kalender()"
       agar Alpine scope tidak terpisah-pisah (bug sebelumnya: calendarGrid dan filter di scope berbeda).
    2. Navigasi bulan mengubah URL dengan reload agar data eventsByDate dari server selalu segar.
       Ini menggantikan pendekatan client-only yang tidak bisa mengambil data bulan lain.
    3. Modal event tidak lagi menggunakan __x internal Alpine v3 (deprecated).
       Diganti dengan $dispatch / $store Alpine yang proper.
    4. Filter kategori bekerja via computed property di dalam scope yang sama.
    5. eventsByDate sudah diisi benar dari server (multi-hari di-expand di controller).
    6. todaysEvents dan upcomingEvents sudah terurut ascending dari controller.
--}}
                        ...
                        @foreach($kategoriList as $key => $info)
                            <button type="button"
                                    ...
                                    :style="isCategoryVisible('{{ $key }}')
                                        ? 'background-color:{{ $info['color'] }}20; color:{{ $info['color'] }}; ring-color:{{ $info['color'] }}'
                                        : 'background-color:#f3f4f6; color:#6b7280'">
                                <span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background-color:{{ $info['color'] }}"></span>
                                {{ $info['label'] }}
                            </button>
                        @endforeach
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
                ...
                <div class="p-4 space-y-2">
                    ...
                    <div class="flex justify-between items-center p-2 rounded-lg bg-red-50 dark:bg-red-900/20">
                        ...
                        <span class="text-sm font-bold text-red-700 dark:text-red-300">{{ $totalExam }}</span>
                    </div>
                    <div class="flex justify-between items-center p-2 rounded-lg bg-green-50 dark:bg-green-900/20">
                        ...
                        <span class="text-sm font-bold text-green-700 dark:text-green-300">{{ $totalLibur }}</span>
                    </div>
                    <div class="flex justify-between items-center p-2 rounded-lg bg-orange-50 dark:bg-orange-900/20">
                        ...
                        <span class="text-sm font-bold text-orange-700 dark:text-orange-300">{{ $totalDeadline }}</span>
                    </div>
                    <div class="flex justify-between items-center p-2 rounded-lg bg-purple-50 dark:bg-purple-900/20">
                        ...
                        <span class="text-sm font-bold text-purple-700 dark:text-purple-300">{{ max(0,$totalLain) }}</span>
                    </div>
                </div>
            </div>
